...
const getNewTrainerQuery = 'SELECT trainerId, name, trainerLvl FROM trainer WHERE trainerId = ?;'
const nicknameExistQuery = 'SELECT name FROM trainer WHERE name =?;'
const trainerIdExistQuery = 'SELECT trainerId FROM trainer WHERE trainerId = ?;'
...
module.exports = {
    checkNewTrainer: (req,res,next) => {
        ...
        try {
            ...
            dbconnection.getConnection((err,connection) => {
                ...
                connection.query(nicknameExistQuery, [nickname], (error,result,fields) => {
                    if (error) next(error)
            ...
            if (err) next(err);
                ...
                connection.query(trainerIdExistQuery, [newTrainerId], (error,result,fields) => {
                    ...
                    if (Object.keys(result).length === 1) {
                        ...
                        count = 10;
                        connection.query(addTrainerQuery, [nickname,pwd,trainerId], (error,result,fields) => {
                            if (error) next(error);
                            logger.info("Check 5 " + trainerId);
                            connection.query(getNewTrainerQuery, [trainerId], (error,result,fields) => {
                                if (error) next(error);
                                connection.release();
                                ...
                                res.status(200).json({
                                    status: 201,
                                    result: result,
                                    message: "Save your trainerId somewhere. You need it to login to your account!"
                                })
                            })
                        })        
                    }
    ...
    pwdChange: (req,res,next) => {
        ...
        res.status(404).json({
            status: 404,
            message: "This is not your account, we can not allow you to change this password"
        })
    }
